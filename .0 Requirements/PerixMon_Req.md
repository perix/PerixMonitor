...
The project has evolved into a stable Dual-Server architecture (Next.js + Python Flask) with Supabase as the data layer.
- **SQL Optimization**: Aggregations and heavy lifting have been moved to PostgreSQL (Materialized Views, Indexes) to overcome Vercel's memory limits.
- **Smart Caching**: Implemented a multi-tier caching strategy (LocalStorage for UI, Server-side filtering for API) ensuring instant navigation.

### 2. Advanced Feature Set
- **Safe Ingestion Protocol**: A "Read-Preview-Write" flow prevents data corruption by allowing users to reconcile changes before committing.
- **Dividend & Expense Management**: Automatic classification of cash flows based on sign, with separate tracking for tax/expenses and dividends.
- **AI Integration**: Experimental support for OpenAI (GPT-5/o-series) to enrich asset metadata and perform web searches.

### 3. UI/UX Enhancements
- **Dynamic Dashboard**: Resizable panels, dual-axis charts, and extensive filtering capabilities.
- **Trend Visualization**: The "Memory" table now features intelligent trend coloring (Green/Red) that activates only above a configurable threshold (default 0.1%), reducing visual noise.
- **Distinction of Historical Assets**: Closed positions are clearly separated from active holdings visually.

### 4. Data Preservation & Backup
- **Full History Backup**: The backup system now exports the **entire price history** along with transactions and settings.
- **Smart Restore**: A robust restore process capable of recreating missing assets and re-mapping dependencies (notes, settings) ensuring no data loss when moving between environments.