...
import { Slider } from '@/components/ui/slider'; // Assuming we have Slider, if not I will use input type range
...
export default function SettingsPanel() {
    ...
    const testAndSave = async () => {
        ...
        try {
            ...
            // 2. Save to Supabase (exclude api_key from DB if desired, but user might want to save it?)
            // Usually API keys shouldn't be valid in frontend DB calls if RLS allows reading.
            // For now, let's assume we save it but be careful. 
            // BrainDock implementation saved it in app_config.
    ...
    const handleResetDatabase = async () => {
        ...
        try {
            ...
            setErrorMessage(error.message || 'Errore durante il reset del database');
            setTestStatus('error');
        } finally {
    ...
    return (
        <div className="max-w-4xl mx-auto pb-20 space-y-8">
            ...
            <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                    <h2 className="text-3xl font-bold tracking-tight text-white">Configurazione AI</h2>
            ...
            <div className="grid grid-cols-1 gap-6">
                ...
                {/* API KEY SECTION - REMOVED FOR SECURITY (Managed via ENV) */}
                {/* 
                <Card className="bg-slate-900/50 border-white/10">
                    <CardHeader>
                        ...
                        <CardDescription>Inserisci la tua chiave API di OpenAI</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        <div className="space-y-2">
                             <Label htmlFor="api_key" className="text-slate-200">OpenAI API Key</Label>
                             <Input 
                                id="api_key"
                                type="password" 
                                placeholder="sk-..." 
                             ...
                             />
                             <p className="text-xs text-slate-500">La chiave verrà salvata cifrata (non implementato ancora, salvata in chiaro nel DB per demo).</p>
                        </div>
                    </CardContent>
                </Card>
                */}
                                ...
                                <AlertDialogContent className="bg-slate-900 border-white/10 text-white">
                                    <AlertDialogHeader>
                                        ...
                                        <AlertDialogDescription className="text-slate-400">
                                            ...
                                            le transazioni e i dati storici dal database. Il tuo account utente verrà mantenuto.
                                        </AlertDialogDescription>
                                    </AlertDialogHeader>
                                    <AlertDialogFooter>
